...
Usage:
  ...
  mio doctor                     # Runs a set of checks to ensure mio installation has what it needs to operate properly
  mio help-search <text>         # Searches mio documentation for the terms provided and lists the results by relevance.
  mio help <term> [<terms>...]   # Show the appropriate documentation page(s) for specified term(s)
  mio (-h | --help)              # Displays this text and exits
  mio (-v | --version)           # Displays mio version and exits
  
  # Project Setup and Configuration
  mio init [<name>] [--template=<name> | --args-file=<path>] [--location=<path>]   # Creates .mio.toml and starts Project creation dialog (unless args-file is specified)
  ...
  # EDA Interface
  mio sim [<ip>] [--test=<name>] [--seed=<integer>] [--verbosity=<level>] [options]  # Performs necessary steps to simulate target IP
  mio regr [<ip>] <regression name> [options]                                         # Executes regression for target IP
  mio lint [<ip>...] [options]   # Launches linting tool against target IPs
  mio synth [<ip>...] [options]   # Launches synthesis tool against target IPs
  mio formal [<ip>...] [options]   # Launches formal verification tool against target IPs
  mio sta     [<ip>...] [options]  # Launches static timing analysis tool against target IPs
  
  # Toolchain
  mio clean [<ip>... | --all]                                        # Deletes artifacts produced by EDA tool(s) for target IP(s)
  mio new [<ip>] [<name>] [--template=<name> | --args-file=<path>]   # Starts Source code template creation dialog
  mio hdl-doc [<ip>...] [--html] [--pdf] [--man]                         # Generates reference documentation for HDL source in target IP(s)
  mio completion [--shell=<variant>]                                 # Prints tab-completion shell code for mio commands and IP cache (ex: mio completion >> ~/.bashrc)
  mio refactor 
  
  # Artifacts/Results Management
  mio results delete [<ip>]               # Deletes all simulation artifacts (including results) for a target IP
  mio results view 
  mio results pack
  mio results publish
  mio results collate
...
Examples:
  ...
  mio init my_chip       --template='mio-empty'    # Creates a new project named 'my_chip' from the template 'mio-empty' in the current location
  ...
  mio results view                                 # View simulation results in default format with default editor
  mio clean                                        # Deletes compilation/elaboration artifacts for default IP
  mio rgr uvmt_ss_mem sanity --cov                 # Runs regression named 'sanity' for IP 'uvmt_ss_mem' with coverage enabled
  mio results view --format=html                   # View regression results in html format with default browser
  mio scrub --all                                  # Deletes all EDA tool outputs for all IPs
"""
